...
Important: the matrix manipulation functions I have provided do NOT
...
(defun net-error (output correct-output)
  ...
  (/ (apply #'+ (mapcar #'(lambda(a) (expt a 2)) (mapcar #'(lambda(a) (elt a 0)) (subtract output correct-output)))) 2))
...
the datum as input."
  (let* ( (i (mapcar #'(lambda(data) (elt data 0)) datum))
          (o (mapcar #'(lambda(x) (map-m #'sigmoid (multiply w (map-m #'sigmoid (multiply v x))))) i)) )
      o))
...
returning a list consisting of new, modified V and W matrices."
  (let* ( (i (mapcar #'(lambda(data) (elt data 0)) datum)) (c (mapcar #'(lambda(data) (elt data 1)) datum)) )
   (setf datum (shuffle datum)) 
   (loop for a from 0 to (- (length datum) 1) 
      do(let* ( (x (elt i a))
          (h (map-m #'sigmoid (multiply v x)))
          (o (map-m #'sigmoid (multiply w h)))
          (y (elt c a)) 
          (d (e-multiply (subtract y o) o (subtract-from-scalar 1 o)))
          (e (e-multiply h (subtract-from-scalar 1 h)))
          (dw (scalar-multiply alpha (multiply d (transpose h))))
          (dv (multiply (scalar-multiply alpha (e-multiply e (multiply (transpose w) d))) (transpose x))) )
          (setf v (add v dv))
          (setf  w (add w dw))))
   (list v w)))
...
(defun early-exit(datum c v w print-all-errors)
  (let* ((o (forward-propagate datum v w)) (worst-err (apply #'max (mapcar #'(lambda(a b) (net-error a b)) c o)))) 
  (if(and print-all-errors t) (format t "~%Errors of Individual Samples: ~S~%" (mapcar #'(lambda(a b) (net-error a b)) c o)))
  (format t "Mean Error of the Training Data = ~S~%" (/ (apply #'+ (mapcar #'(lambda(a b) (net-error a b)) c o)) (length datum)))
  (format t "Worst Error of the Training Data = ~S~%" worst-err)
  (if(< worst-err *a-good-minimum-error*) t nil)))
...
and the final W matrix of the learned network."
  (let* ( (datum data) (vw)(v (make-random-matrix num-hidden-units (length (elt (elt datum 0) 0)) initial-bounds))
    (w (make-random-matrix (length (elt (elt datum 0) 1)) num-hidden-units initial-bounds)) (c (mapcar #'(lambda(a) (elt a 1)) datum)) )
    (loop for a from 1 to max-iterations
      until(if(= 0 (mod a modulo)) (early-exit datum c v w print-all-errors))
      do(setf vw (back-propagate datum alpha v w))
      do(setf v (elt vw 0))
      do(setf w (elt vw 1)))
    (list v w)))
...
and use a modulo of MAX-ITERATIONS."
  (let* ((datum (convert-data data)) (vw)(middle (if(= 0 (mod (length datum) 2)) (/ (length datum) 2) (/ (- (length datum) 1) 2))) 
    (train-data (subseq datum 0 middle))
    (validation (subseq datum middle (length datum)))
    (c (mapcar #'(lambda(a) (elt a 1)) validation)) 
    (vw (net-build train-data num-hidden-units alpha initial-bounds max-iterations 250 nil)))
    (/ (apply #'+ (mapcar #'(lambda(a b) (net-error a b)) c (forward-propagate validation (elt vw 0) (elt vw 1)))) (length validation))))
...
and use a modulo of MAX-ITERATIONS."
 (let* ( (datum (convert-data data)) (n (length datum)) (chunk-size) (prev-index 0) (sum-error 0) (test-data) (train-data) (vw) )
  (loop for i from 1 to k
    do(if(<= i (mod n k)) (setf chunk-size (+ 1 (floor (/ n k)))) (setf chunk-size (floor (/ n k))))
    do(setf test-data (subseq datum prev-index (+ prev-index chunk-size)))
    do(setf train-data (append (subseq datum 0 prev-index) (subseq datum (min (+ prev-index chunk-size) n) n)))
    do(setf vw (net-build train-data num-hidden-units alpha initial-bounds max-iterations 250 nil)) 
    do(setf sum-error (+ sum-error (/ (apply #'+ (mapcar #'(lambda(a b) (net-error a b)) 
                            (mapcar #'(lambda(a) (elt a 1)) test-data) (forward-propagate test-data (elt vw 0) (elt vw 1)))) (length test-data))))
    do(setf prev-index (+ prev-index chunk-size)))
    (/ sum-error k)))
